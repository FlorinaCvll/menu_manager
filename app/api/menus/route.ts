...
export async function POST(request: Request) {
  ...
  const fechaMenu = fechaDesdeInput(String(body.fecha));
  ...
  const menu = await prisma.$transaction(async (tx) => {
    ...
    const guardado = await tx.menu.upsert({
      ...
      update: {
        ...
        precioMedio: Number(body.precioMedio),
        precioTerraza: Number(body.precioTerraza),
        datosAdicionales: body.datosAdicionales || null,
      },
      create: {
        fecha: fechaMenu,
        precio: Number(body.precio),
  ...
  revalidatePath("/camarero/menuDia");
